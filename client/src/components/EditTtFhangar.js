...
const EditTtFhangar = ({isLoggedIn, setIsLoggedIn}) => {
...
useEffect(()=>{axios.get(`http://localhost:8000/api/tTfHangar/${id}`, {withCredentials:true})
.then((res)=>{
    console.log(res)
...
const updateHangarList = (e) =>{
    e.preventDefault()
    axios.put(`http://localhost:8000/api/tTfHangar/${id}`, {
        area,
        locationOfPlane,
        tailNumber,
        fuelType,
    ...
    .then((res)=>{
        ...
        navigate("/accountList/towerLocations/thirteenThroughFifteenHangars")
    })
}
...
.then((res)=>{
  ...
  return (
    ...
    <div className='display-flex3'>
        <form onSubmit={updateHangarList}>
        ...
        <Link to={'/accountList/towerLocations/thirteenThroughFifteenHangars'} className="homeBtn">Back</Link>
