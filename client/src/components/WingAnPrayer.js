...
const WingAnPrayer = ({isLoggedIn, setIsLoggedIn}) => {

const [wingAnPrayer, setWingAnPrayer]= useState([]);
const [user, setUser] = useState()
    // const {airRogerList, setAirRogerList}= props;
  
    const {REACT_APP_API_KEY} = process.env;
    
    useEffect(()=>{axios.get("http://localhost:8000/api/waap", {withCredentials:true})
      .then((res)=>{
  
        console.log(res.data)
        setWingAnPrayer(res.data)
      })
...
<div className="header">
  ...
  <div class="header-right">
    ...
    <Link to={'/wingPrayerCreate'} className="active">Add a tail #</Link>
...
<table className="table">
  ...
  <tbody>
    ...
    <tr key ={plane._id}>
      ...
      <td>
        <button><Link to={`/wingPrayer/${plane._id}`}>View</Link></button> 
        <button><Link to={`/wingPrayer/edit/${plane._id}`}>Edit</Link></button>
