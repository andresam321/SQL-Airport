...
const TshadeRamp = ({isLoggedIn, setIsLoggedIn}) => {


    const [tShade, setTshadeRamp]= useState([]);
    const [tailNumber, setTailNumber] = useState([])
    const [user, setUser] = useState()
    const {REACT_APP_API_KEY} = process.env;
    
    useEffect(()=>{axios.get("http://localhost:8000/api/tShades", {withCredentials:true})
      .then((res)=>{
        console.log(res)
        console.log(res.data)
        setTshadeRamp(res.data)
      })
...
<div className="header">
  ...
  <div class="header-right">
    ...
    <Link to={'/tShadeRampCreate'} className="active">Add a tail # </Link>
...
<table className="table">
  ...
  <tbody>
    ...
    <tr key ={plane._id}>
      ...
      <td>
        <button><Link to={`/tShadeRamp/${plane._id}`}>View</Link></button> 
        <button><Link to={`/tShadeRamp/edit/${plane._id}`}>Edit</Link></button>
