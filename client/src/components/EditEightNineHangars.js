...
const EditEightNineHangars = ({isLoggedIn, setIsLoggedIn}) => {
  ...
  useEffect(()=>{axios.get(`http://localhost:8000/api/eightNineHangars/${id}`)
  .then((res)=>{
      console.log(res)
  ...
  const updateEightNineHangars = (e) =>{
      e.preventDefault()
      axios.put(`http://localhost:8000/api/eightNineHangars/${id}`, {
          area,
          locationOfPlane,
          tailNumber,
          fuelType,
      ...
      .then((res)=>{
          ...
          navigate("/accountList/terminalLocations/eightNineHangars")
      })
  }
...
<form onSubmit={updateEightNineHangars}>
        ...
        <Link to={'/accountList/terminalLocations/eightNineHangars'} className="homeBtn">Back</Link>
...
export default EditEightNineHangars