...
const TenThroughEleven = ({isLoggedIn, setIsLoggedIn}) => {

  const [tenEleven, setTenEleven]= useState([]);
  const [user, setUser] = useState()
  // const {airRogerList, setAirRogerList}= props;

  const {REACT_APP_API_KEY} = process.env;
  
  useEffect(()=>{axios.get("http://localhost:8000/api/tteHangars",{withCredentials:true})
    .then((res)=>{

      console.log(res.data)
      setTenEleven(res.data)
    })
...
<div className="header">
  ...
  <div class="header-right">
    ...
    <Link to={'/tenElevenCreate'} className="active">Add a tail #</Link>
...
<table className="table">
  ...
  <tbody>
    ...
    <tr key ={plane._id}>
      ...
      <td>
        <button><Link to={`/tenElevenHangar/${plane._id}`}>View</Link></button> 
        <button><Link to={`/tenElevenHangar/edit/${plane._id}`}>Edit</Link></button>
...
export default TenThroughEleven