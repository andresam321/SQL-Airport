...
const CreateNorthTower = ({isLoggedIn, setIsLoggedIn}) => {
    ...
    const submitHandler = (e) =>{
        e.preventDefault()
        axios.post('http://localhost:8000/api/northTowerArea', {
            locationOfPlane,
            area,
            tailNumber,
            airplaneType,
        ...
        // setPositivePrist('NA')
        // setCheckName('')
        navigate('/accountList/terminalLocations/northTower')
        })
        .catch((err)=>{
            console.log(err)
...
export default CreateNorthTower