...
const EditNorthTower = ({isLoggedIn, setIsLoggedIn}) => {
    ...
    useEffect(()=>{axios.get(`http://localhost:8000/api/northTower/${id}`,{withCredentials:true})
    .then((res)=>{
        console.log(res)
    ...
    const updateNorthTower = (e) =>{
        e.preventDefault()
        axios.put(`http://localhost:8000/api/northTower/${id}`, {
            area,
            locationOfPlane,
            tailNumber,
            fuelType,
...
<div>
    ...
    <div className='display-flex3'>
         <form onSubmit={updateNorthTower}>
