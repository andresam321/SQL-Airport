...
const SouthTower = ({isLoggedIn, setIsLoggedIn}) => {

  const [southTower, setSouthTower]= useState([]);
  const [user, setUser] = useState()
  // const {airRogerList, setAirRogerList}= props;

  const {REACT_APP_API_KEY} = process.env;
  
  useEffect(()=>{axios.get("http://localhost:8000/api/southTower")
    .then((res)=>{

      console.log(res.data)
      setSouthTower(res.data)
    })
...
<div className="header">
  ...
  <div class="header-right">
    ...
    <Link to={`/southTowerCreate`} className="active">Add a tail #</Link>
...
<table className="table">
  ...
  <tbody>
    ...
    <tr key ={plane._id}>
      ...
      <td>
        <button><Link to={`/southTower/${plane._id}`}>View</Link></button> 
        <button><Link to={`/southTower/edit/${plane._id}`}>Edit</Link></button>
