...
const EditTenElevenHangar = ({isLoggedIn, setIsLoggedIn}) => {
    const{id} = useParams();
    ...
    useEffect(()=>{axios.get(`http://localhost:8000/api/tteHangar/${id}`)
    .then((res)=>{
        console.log(res)
    ...
    const updateTenEleven = (e) =>{
        e.preventDefault()
        axios.put(`http://localhost:8000/api/tteHangar/${id}`, {
            area,
            locationOfPlane,
            tailNumber,
            fuelType,
        ...
        .then((res)=>{
            ...
            navigate("/accountList/towerLocations/tenEleven")
        })
    }
...
<form onSubmit={updateTenEleven}>
        ...
        <Link to={'/accountList/towerLocations/tenEleven'} className="homeBtn">Back</Link>
...
export default EditTenElevenHangar