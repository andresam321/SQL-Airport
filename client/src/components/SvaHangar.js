...
const SvaHangar = ({isLoggedIn, setIsLoggedIn}) => {
  const [svaHangar, setSvaHangar]= useState([]);
  const [user, setUser] = useState()
  // const {airRogerList, setAirRogerList}= props;

  const {REACT_APP_API_KEY} = process.env;
  
  useEffect(()=>{axios.get("http://localhost:8000/api/svaHangar")
    .then((res)=>{

      console.log(res.data)
      setSvaHangar(res.data)
    })
...
<div className="header">
  ...
  <div class="header-right">
    ...
    <Link to={`/svaHangarCreate`} className="active">Add a tail #</Link>
...
<table className="table">
  ...
  <tbody>
    ...
    <tr key ={plane._id}>
      ...
      <td>
        <button><Link to={`/svaHangar/${plane._id}`}>View</Link></button> 
        <button><Link to={`/svaHangar/edit/${plane._id}`}>Edit</Link></button>
