...
const SkywayHolding = ({isLoggedIn, setIsLoggedIn}) => {

  const [skywayHolding, setSkywayHolding]= useState([]);
  const [user, setUser] = useState()
  // const {airRogerList, setAirRogerList}= props;

  const {REACT_APP_API_KEY} = process.env;
  
  useEffect(()=>{axios.get("http://localhost:8000/api/skywayHoldings")
    .then((res)=>{

      console.log(res.data)
      setSkywayHolding(res.data)
    })
...
<div className="header">
  ...
  <div class="header-right">
    ...
    <Link to={'/skywayHoldingCreate'} className="active">Add a tail #</Link>
...
<table className="table">
  ...
  <tbody>
    ...
    <tr key ={plane._id}>
      ...
      <td>
        <button><Link to={`/skywayHolding/${plane._id}`}>View</Link></button> 
        <button><Link to={`/skywayHolding/edit/${plane._id}`}>Edit</Link></button>
