...
const EditSkywayHolding = ({isLoggedIn, setIsLoggedIn}) => {
    const{id} = useParams();
    ...
    useEffect(()=>{axios.get(`http://localhost:8000/api/skywayHolding/${id}`)
    .then((res)=>{
        console.log(res)
    ...
    const updateSkywayHolding = (e) =>{
        e.preventDefault()
        axios.put(`http://localhost:8000/api/skywayHolding/${id}`, {
            area,
            locationOfPlane,
            tailNumber,
            fuelType,
        ...
        .then((res)=>{
            ...
            navigate("/accountList/towerLocations/skywayHolding")
        })
    }
  ...
  return (
    ...
    { user ? (
        <div className='display-flex3'>
            <form onSubmit={updateSkywayHolding}>
        ...
        <Link to={'/accountList/towerLocations/skywayHolding'} className="homeBtn">Back</Link>
...
export default EditSkywayHolding