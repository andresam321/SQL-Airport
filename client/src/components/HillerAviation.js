...
const HillerAviation = ({isLoggedIn, setIsLoggedIn}) => {
    const [hillerAviation, setHillerAviation]= useState([]);
    const [tailNumber, setTailNumber] = useState([])
    const {REACT_APP_API_KEY} = process.env;
    const [user, setUser] = useState()
    
    useEffect(()=>{axios.get("http://localhost:8000/api/hillerAviation")
      .then((res)=>{
        console.log(res)
        console.log(res.data)
        setHillerAviation(res.data)
      })
...
<div className="header">
  ...
  <div class="header-right">
    ...
    <Link to={'/hillerAviationCreate'} className="active">Add a tail # </Link>
...
<table className="table">
  ...
  <tbody>
    {hillerAviation.map((plane,index)=>{
    ...
    <tr key ={plane._id}>
      ...
      <td>
        <button><Link to={`/hillerAviation/${plane._id}`}>View</Link></button> 
        <button><Link to={`/hillerAviation/edit/${plane._id}`}>Edit</Link></button>
